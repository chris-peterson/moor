...
- **[CO-06]** When the user opens a comment composer (Space / Enter on the current hunk per NV-07, a range selection per CO-04, or a changeset / file control per CO-05), the viewer shall present an auto-growing text area with an action control; Enter shall confirm, Shift+Enter shall insert a newline, Escape shall confirm a non-empty comment or discard an empty one, and Tab shall down-classify the action (`fix-now` → `fix-later` → `consider` → `fix-now`) with Shift+Tab walking back up.
