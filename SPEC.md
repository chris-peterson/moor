...
| **action** | A comment's severity, from the review-sidecar contract's shared vocabulary: `must-fix` (must be addressed before shipping — the only tier that gates the exit code), `suggestion` (a recommended change), `nit` (a trivial/style point), or `question` (a query for the author). `suggestion` / `nit` / `question` are advisory. Deferral ("address later") is expressed by handing a comment to a follow-up skill (e.g. `/issue`), not a distinct action. |
...
- **[NV-17]** When the user presses `d` (either case), the viewer shall toggle the input details panel (IM.IN-01) — the keyboard companion to clicking the change region.
...
- **[NV-19]** When the user presses `n` (either case) or activates the comments control, the viewer shall open the comments panel to manage existing comments (CO-08).
...
- **[NV-22]** When the user presses `m` (either case), the viewer shall begin a comment on the commit message (CO-09) — expanding the details panel and opening the comments panel to type it.
- **[NV-23]** When the user presses `c` (either case), the viewer shall begin a comment on the overall changeset (CO-05) — opening the comments panel to type it.
...
- **[DD-17]** When the user approves with one or more unreviewed hunks and no `must-fix` comments, the viewer shall confirm first, stating the count of unreviewed hunks (not a viewed/total ratio). The low-friction default action (focused, primary) shall resume the review where the reviewer left off; finalizing the review as a clean approve (exit code 0) shall be the secondary action.
...
- **[CO-01]** The viewer shall represent review feedback as **comments**. A comment consists of a free-text `body`, an `action`, and a target. Comments reconcile the former per-hunk rejections (now `must-fix` comments) and free-text notes (now advisory `suggestion` / `nit` / `question` comments).
- **[CO-02]** A comment's target shall be one of: the overall **changeset** (unanchored), the **commit message** (CO-09), a **file**, or a **line range** within a file. The range may cover changed or unchanged (context) lines, so the reviewer can flag an issue in neighboring code, not only in the diff's hunks.
- **[CO-03]** When a comment is created, the viewer shall default its action to `must-fix`, and shall let the user set any comment's action to `must-fix`, `suggestion`, `nit`, or `question`. `must-fix` denotes feedback that must be addressed before shipping and gates the exit code (EC-02); `suggestion`, `nit`, and `question` are advisory and shall not affect review completion or the exit code.
- **[CO-04]** When the user presses on the **new (right) side's** line-number gutter, the gesture shall resolve on release: a quick press-and-release on a **changed** line toggles its reviewed state (NV-06), while on an **unchanged (context)** line it opens a single-line comment (a context line has no review state); a press that drags across lines selects the spanned range; and a press held in place past a long-press threshold selects the single line. A range or single-line selection shall open a comment composer (CO-06) anchored to it. On hover, the new-side gutter shall reveal a **+** control giving line commenting a visible affordance on par with the file / changeset / commit-message controls (CO-05, CO-09) rather than relying on the gesture or context menu alone: clicking it comments the single line (changed or context), and dragging from it selects a range — the same press/drag gesture as the gutter, except a plain release always comments rather than toggling review. The old (left) side's gutter is not a comment affordance — review feedback references the new file. Right-clicking any line offers the same single-line comment via the context menu (CM-04).
- **[CO-05]** The viewer shall provide, for each non-line target, a control that creates a comment on that target, **inferring the target from the control's surface**: the overall **changeset** (a control on the header's change region, and the `c` / `C` key per NV-23), the **commit message** (CO-09), and a **file** (a control on its file header). The reviewer does not pick a target from a list — the surface the control sits on determines it.
- **[CO-09]** The viewer shall let the reviewer target a comment at the **commit message** — a changeset-level annotation about the message itself, so the message stays inside the same feedback loop as the code. It shall be creatable from the expanded details panel (an annotation control there, and the `m` / `M` key per NV-22), following the same action model as any other comment (CO-03); a `must-fix` commit-message comment gates the exit code (EC-02) like any other. The expanded details panel shall list the message's existing comments beside the message.
- **[CO-06]** When the user opens a comment composer (Space / Enter on the current hunk per NV-07, a range selection per CO-04, or a changeset / file control per CO-05), the viewer shall present an auto-growing text area with an action control, following the text-entry conventions (TE-01..03). For this composer specifically, Escape confirms a non-empty comment and discards an empty one (TE-02), and Tab down-classifies the action `must-fix` → `suggestion` → `nit` → `question` → `must-fix`, with Shift+Tab walking back up (TE-03).
...
Conventions for any field the reviewer types into. The keyboard contract — Enter, Escape, and Tab — is uniform across composers, the comments panel, and future inputs, so muscle memory carries between surfaces.

- **[TE-01]** When the user presses Enter within a text input paired with a primary or confirm action — the inline comment composer's confirm (CO-06) or the comments panel's "Done" (CO-08) — the viewer shall activate that action; Shift+Enter shall insert a newline. A text input with no associated primary action retains Enter as a newline. (Button-only dialogs follow DD-14, where Enter activates the focused button.)
- **[TE-02]** When the user presses Escape within a text input, the viewer shall dismiss it: a composer keeps a non-empty entry and discards an empty one (CO-06); a panel or dialog closes (CO-08, DD-14).
- **[TE-03]** Where a text input is paired with a cycling control — the comment composer's action selector — Tab shall advance the control and Shift+Tab shall reverse it (CO-06).

### Review Feedback (RV)

- **[RV-01]** When all hunks have been reviewed, the viewer shall display a transient "Review Complete!" notification
- **[RV-04]** While a file has one or more `must-fix` comments, the viewer shall display that file's sidebar entry in red
...
- **[EC-01]** When closed with all hunks reviewed and no `must-fix` comments, the application shall exit with code 0
- **[EC-02]** When closed with one or more `must-fix` comments, the application shall exit with code 1
- **[EC-03]** When closed with one or more unreviewed hunks and no `must-fix` comments, the application shall exit with code 2
...
- **[IM-01]** The context channel shall be resolved by checking, in order: the `--context <path>` CLI flag, then the `REVIEW_CONTEXT` environment variable. The viewer shall read the file's `input` section on launch and write its `output` section during and after the review.
- **[IM-02]** When no context channel is configured (neither `--context` nor `REVIEW_CONTEXT` is set), the viewer shall display a warning banner in the header so the user can investigate the tooling disconnect rather than discovering it at exit time. The banner shall include a hint that the user can pass `--context <path>` to capture review output.

#### Inputs (caller → viewer)

- **[IM.IN-01]** The input section shall contain a `title` string and a `details` array of `{label, value}` rows. The viewer shall render `title` as the change headline and shall toggle the expandable details when the user clicks anywhere in the change region (other than its interactive controls) or presses the `d` / `D` keys (NV-17). The caller decides how content maps to these fields — a single commit, a range of commits, or a non-commit context all use the same shape.
- **[IM.IN-02]** The viewer shall present the change as a label-less header: an always-visible **location** eyebrow (`project @ branch`, composed from the repository and branch detail rows) and the commit-message **headline** (the `title`); when the caller supplies a beacon **task**, it shall be emphasized in the eyebrow. Expanding shall reveal the rest of the change — the full commit-message body (a `body` / `message` / `description` row, rendered as message prose, not a labeled field), a provenance grid of the remaining `details` rows (excluding low-signal rows the viewer suppresses, e.g. `range`), and the commit-message annotation control with any existing message comments beside the message (CO-09). The layout shall degrade gracefully when a source row is absent.
...
- **[IM.OUT-02a]** The output section shall always include `reviewer` (string, from `git config user.name`) and `comments` (array). Each comment is `{ body, action, target?, file?, startLine?, endLine? }`: a changeset comment omits `file` and `target`; a commit-message comment (CO-09) includes `target: "commit-message"` and omits `file`; a file comment includes `file`; a line-range comment includes `file`, `startLine`, and `endLine`. `action` is one of `must-fix`, `suggestion`, `nit`, `question`. The calling agent interprets the comments.
...
- **[IM.OUT-03]** While `must-fix` comments exist, the viewer shall display one badge per affected file in the header's output region, each showing the file's `must-fix` count; when the user clicks a badge, the viewer shall navigate to that file's first `must-fix` comment.
- **[IM.OUT-04]** The viewer shall display review progress ("X of Y changes viewed") in the header's output region.
- **[IM.OUT-05]** While in single-file mode (no directory sidebar), the viewer shall display an equivalent review-progress footer at the bottom of the view: a progress bar with "X of Y changes viewed" (collapsing to "All changes viewed · q to close" when complete) and a `must-fix` count when such comments exist.
