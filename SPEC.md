...
| **comment** | Review feedback anchored to the changeset, a file, or a line range. Carries a free-text body and nothing else — comments are ungraded. Reconciles the former rejections and notes. |
...
- **[DD-12]** When the user attempts to close with one or more comments, the viewer shall display a send-feedback confirmation dialog revealing every comment grouped by location (per-location count and each comment's body), offering both outcomes: "Send review feedback" (EC-02) and "Approve anyway" (EC-01, sending the same comments as notes). Which is the primary CTA shall follow how the reviewer arrived — approving leads with "Approve anyway", every other route leads with "Send review feedback".
...
- **[DD-17]** When the user approves with one or more unreviewed hunks and no comments, the viewer shall confirm first, stating the count of unreviewed hunks (not a viewed/total ratio). The low-friction default action (focused, primary) shall resume the review where the reviewer left off; finalizing the review as a clean approve (exit code 0) shall be the secondary action.
...
- **[CO-01]** The viewer shall represent review feedback as **comments**. A comment consists of a free-text `body` and a target. Comments reconcile the former per-hunk rejections and free-text notes.
...
- **[CO-03]** The viewer shall not grade comments. There is no severity, priority, or disposition field: every comment is feedback the author is expected to address, and any comment gates the exit code (EC-02). A reviewer who wants a comment delivered without blocking approves anyway from the send-feedback dialog (DD-12); deferral ("address later") is expressed by handing the comment to a follow-up skill (e.g. `/issue`), not by a tier the viewer models.
...
- **[CO-09]** The viewer shall let the reviewer target a comment at the **commit message** — a changeset-level annotation about the message itself, so the message stays inside the same feedback loop as the code. It shall be creatable from the expanded details panel (an annotation control there, and the `m` / `M` key per NV-22), a commit-message comment gates the exit code (EC-02) like any other (CO-03). The expanded details panel shall list the message's existing comments beside the message.
- **[CO-06]** When the user opens a comment composer (Space / Enter on the current hunk per NV-07, a range selection per CO-04, or a changeset / file control per CO-05), the viewer shall present an auto-growing text area, following the text-entry conventions (TE-01, TE-02). For this composer specifically, Escape confirms a non-empty comment and discards an empty one (TE-02).
...
- **[RV-04]** While a file has one or more comments, the viewer shall display that file's sidebar entry in red
...
- **[EC-01]** When closed with all hunks reviewed and no comments, the application shall exit with code 0. Approving anyway (DD-12, DD-13) shall also exit 0, whatever the comment or unreviewed count.
- **[EC-02]** When closed with one or more comments, the application shall exit with code 1
- **[EC-03]** When closed with one or more unreviewed hunks and no comments, the application shall exit with code 2
...
- **[IM.OUT-02a]** The output section shall always include `reviewer` (string, from `git config user.name`) and `comments` (array). Each comment is `{ body, target?, file?, startLine?, endLine? }`: a changeset comment omits `file` and `target`; a commit-message comment (CO-09) includes `target: "commit-message"` and omits `file`; a file comment includes `file`; a line-range comment includes `file`, `startLine`, and `endLine`. Comments carry no severity field (CO-03). The calling agent interprets the comments.
...
- **[IM.OUT-03]** While comments exist, the viewer shall display one badge per affected file in the header's output region, each showing the file's comment count; when the user clicks a badge, the viewer shall navigate to that file's first comment.
- **[IM.OUT-04]** The viewer shall display review progress ("X of Y changes viewed") in the header's output region.
- **[IM.OUT-05]** While in single-file mode (no directory sidebar), the viewer shall display an equivalent review-progress footer at the bottom of the view: a progress bar with "X of Y changes viewed" (collapsing to "All changes viewed · q to close" when complete) and a comment count when comments exist.
