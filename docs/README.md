...
4. **You comment where it matters.** `Space` anchors a comment to the current
   change. Comments are ungraded — there's no severity to pick, because every
   comment is something you want addressed.
5. **You give a verdict and close.** **Approve** finalizes clean; **Reject**
   opens the feedback dialog, seeding a comment if you haven't left one. `q`
   closes. Leaving comments *and* approving is a deliberate choice the dialog
   offers, for when a note isn't worth blocking on.
6. **Your review flows back into the agent's context.** moor streams the
   ...
   `1` when you sent comments, `2` unreviewed, `3` closed early. Claude reads
   your comments as concrete, line-anchored instructions and addresses them.
...
On close, the send-feedback dialog lists every comment before it streams back to
the agent, and carries both outcomes — sending the feedback (exit `1`) or
approving anyway (exit `0`) with the same comments delivered as notes.
...
reflects the outcome (clean `0`, comments sent `1`, unreviewed `2`, early
...
`{body, file?, startLine?, endLine?}` — flushing on every state change. If the reviewer
