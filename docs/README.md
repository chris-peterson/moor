...
1. **Claude finishes a change and hands it to moor.** You don't reach for
   ...
   upstream caller wires up the `REVIEW_CONTEXT` file, launches moor, and reads
...
4. **You comment where it matters.** `Space` anchors a comment to the current
   change. Each comment carries an action tier — **must-fix** (blocks the
   change), **suggestion**, **nit**, or **question** — so "this is wrong" and
   "nice someday" don't read the same. `Tab` in the composer cycles the tier.
...
6. **Your review flows back into the agent's context.** moor streams the
   ...
   `1` when any comment is must-fix, `2` unreviewed, `3` closed early. Claude
   reads your `must-fix` comments as concrete, line-anchored instructions and
   addresses them.
...
![Anchoring a comment to a change in moor: the inline composer with its must-fix / suggestion / nit / question action tiers, the targeted change banded to match, and the Approve / Reject verdict buttons in the status row](composer.png)

On close, the send-feedback dialog lists every comment with its action tier
before it streams back to the agent — a single `must-fix` here, which sets the
exit code to `1`:

![moor's send-feedback dialog on close: "1 must-fix comment of 1 total across 1 location", the comment shown under right.js with its must-fix badge, and a "Send review feedback" button](feedback-dialog.png)
...
reflects the outcome (clean `0`, a `must-fix` comment `1`, unreviewed `2`, early
...
at a channel with the `--context` flag or the `REVIEW_CONTEXT` environment
variable:
...
`{body, action, file?, startLine?, endLine?}` where `action` is `must-fix`,
`suggestion`, `nit`, or `question` — flushing on every state change. If the reviewer
